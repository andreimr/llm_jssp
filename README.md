...
| `/attach <path>` | attach a PDF or image (PNG/JPEG/GIF/WebP) to your next message |
...
- `optimist/tools/sandbox.py` — solver code runs in a subprocess with wall-clock
  timeout plus CPU/memory rlimits. It is a resource sandbox, not a security
  boundary.
...
The test suite covers the LP solver path, the sandbox (including timeouts and
OR-Tools importability), provider wire-format conversion for both backends, and
the agent loop end-to-end against a scripted fake provider.
